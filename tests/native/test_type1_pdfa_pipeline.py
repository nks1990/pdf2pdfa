...
from tests.native.test_type1_core import _font
from tests.native.test_type1_pdf_render import _type1_font


def _transparent_type1_pdf() -> bytes:
    builder = PDFBuilder(version="1.7")
    font_ref = builder.add(_type1_font(_font(), encoding=PDFName("WinAnsiEncoding")))
    resources = PDFDict(
        {
...
def test_pdfa1_pipeline_flattens_transparent_type1_text_with_visual_fidelity(tmp_path: Path):
    output = tmp_path / "type1-archive.pdf"
    result = OwnedPDFAPipeline(
        fidelity="auto",
        transparency_dpi=72,
        visual_dpi=72,
    ).convert(_transparent_type1_pdf(), output, level="1b")
