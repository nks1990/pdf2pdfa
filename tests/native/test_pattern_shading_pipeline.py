...
def _transparent_pattern_pdf() -> bytes:
    ...
    contents = builder.add(
        PDFStream(
            PDFDict(),
            b"/Pattern cs /P scn 0 0 100 20 re f\n",
        )
    )
...
def test_pdfa1_pipeline_flattens_transparent_shading_pattern(tmp_path: Path):
    output = tmp_path / "archive.pdf"
    result = OwnedPDFAPipeline(
        fidelity="auto",
        transparency_dpi=72,
        visual_dpi=72,
    ).convert(_transparent_pattern_pdf(), output, level="1b")
