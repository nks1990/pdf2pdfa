...
This is the production implementation behind :mod:`pdf2pdfa.native.type1`.
...
def _ascii_hex_payload(data: bytes) -> bytes | None:
    ...
    if len(digits) & 1:
        digits.pop()  # trailing hex nibble from cleartext such as 'cleartomark'
    try:
...
class _Interpreter:
    ...
    def _execute(self, data: bytes, *, depth: int, is_subr: bool) -> str:
        ...
        pos = 0
        large_div_stage = 0  # 1: large numerator seen; 2: denominator seen
        while pos < len(data):
            self._tick()
